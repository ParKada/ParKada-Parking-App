...
serve(async (req) => {
  ...
  try {
    ...
    const { application_id, staff_id, new_name } = await req.json();
    if (!application_id || !staff_id || !new_name) {
      return new Response(
    ...
    // 4. Update Staff Name via Supabase Auth Admin API
    ...
    // 5. Update admin_profiles table as well
    ...
    return new Response(
      JSON.stringify({ success: true, message: "Staff name updated successfully!" }),
