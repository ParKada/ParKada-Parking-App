...
def run_ocr_validation(slot_id, is_reservable, camera_id, raw_frame, bbox, display_shape):
    def _task():
        ...
        crops_to_try = []
        results = plate_model(vehicle_crop, conf=0.15, verbose=False)
    ...
    # --- Fallback if no plate was read ---
    clean_text = "UNREADABLE"
    print(f"[OCR] No plate detected in slot {slot_id[:8]}, logging as UNREADABLE")
    log_plate_validation(clean_text, 0.0, camera_id)
    ...
    if is_reservable:
        ...
        msg = f"Unknown vehicle parked at reservable slot {slot_label} without reservation."
        print(f"[OCR] ❌ MISMATCH: {msg}")
        send_admin_notifications(TARGET_LOT_ID, "Reservation Mismatch", msg)
        
        def patch_mismatch_fallback():
            try:
                patch_url = f"{VITE_SUPABASE_URL}/rest/v1/plate_validation_logs?detected_plate=eq.UNREADABLE&order=created_at.desc&limit=1"
                patch_req = urllib.request.Request(patch_url, data=json.dumps({"validation_status": "mismatched"}).encode("utf-8"), method="PATCH")
                patch_req.add_header("apikey", VITE_SUPABASE_SERVICE_KEY)
                patch_req.add_header("Authorization", f"Bearer {VITE_SUPABASE_SERVICE_KEY}")
                patch_req.add_header("Content-Type", "application/json")
                ...
                print(f"[OCR] [FAIL] Could not patch mismatch status for unreadable: {e}")
        
        threading.Timer(2.0, patch_mismatch_fallback).start()
...
def api_call():
    try:
        # 1. Fetch admins for this lot OR super_admins
        url = f"{VITE_SUPABASE_URL}/rest/v1/admin_profiles?or=(assigned_lot_id.eq.{lot_id},role.eq.super_admin)&select=id"
        req = urllib.request.Request(url)
        req.add_header('apikey', VITE_SUPABASE_SERVICE_KEY)
        req.add_header('Authorization', f'Bearer {VITE_SUPABASE_SERVICE_KEY}')
