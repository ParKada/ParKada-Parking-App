...
logs = api_request("plate_validation_logs?select=id,linked_walk_in_id,linked_reservation_id,confidence_score")

tasks = []
for log in logs:
    if log.get('linked_walk_in_id'):
        if log.get('confidence_score', 100) < 60:
            tasks.append((log['id'], 'manual_review'))
        else:
...
print(f"Updating {len(tasks)} OCR logs...")
with ThreadPoolExecutor(max_workers=50) as executor:
    executor.map(update_log, tasks)

print("Done updating OCR logs to 'detected'!")
