      ...
      <View className="relative bg-[#0A1D37] overflow-hidden flex-1">
        ...
        >
          <Image 
            ...
            className="w-20 h-20 rounded-3xl mb-3 border-2 border-white/10"
          />
          <Text className="text-4xl font-black text-white text-center tracking-tighter">
            Par<Text className="text-amber-400">Kada</Text>
          </Text>
          <Text className="text-white/70 text-xs text-center mt-1 font-bold uppercase tracking-widest">
            Lipa City Downtown
          </Text>
        </View>
      </View>
          ...
          {features.map(({ icon: Icon, title, desc }) => (
            <View key={title} className="flex-row items-center gap-4 py-1">
              ...
              <View className="flex-1 justify-center">
                <Text className="text-sm font-black text-slate-800 tracking-tight leading-snug">{title}</Text>
                <Text className="text-[11px] text-slate-400 font-bold mt-1 leading-normal">{desc}</Text>
              </View>
            </View>
          ))}
        </View>
